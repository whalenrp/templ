...
const (
	...
	contextModeConcurrent                        // Safe path: sync.Map, concurrent-safe
)
...
func ConcurrentSeq(seq iter.Seq[Component]) Component {
	return ComponentFunc(func(ctx context.Context, w io.Writer) error {
		// Create concurrent child context (internal API)
		concurrentCtx := newConcurrentChildContext(ctx)
			...
			go func() {
				...
				for res := range jobs {
					...
					func() {
						...
						res.err = res.component.Render(concurrentCtx, res.buffer)
					}()
					close(res.done)
				}
			}()
...
func Concurrent(components ...Component) Component {
	...
	// Delegate to ConcurrentSeq using slices.Values
	return ConcurrentSeq(slices.Values(components))
}
